...
def save_data(avm_dir, output_dir):
    ...
    for img_filename in filename_list:
        ...
        det_anno_files = sorted(os.listdir(avm_det_anno_file))

        print(f'\nProcessing {img_filename}: ')
        progress_bar = mmcv.ProgressBar(len(img_files))
        for img_name, anno_name in zip(img_files, det_anno_files):
            # assert img_name.split('.')[0] == anno_name.split('.')[0]
            # for debug
            if not img_name.split('.')[0] == anno_name.split('.')[0]:
                target_anno_name = img_name.split('.')[0] + '.json'
                if target_anno_name not in det_anno_files:
                    continue
                else:
            ...
            save_img_file = img_filename + '_' + img_name.split('.')[0] + '.png'
